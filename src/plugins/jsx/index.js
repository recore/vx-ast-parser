...
import { isIdentifierChar, isIdentifierStart, isSpecialChar } from "../../util/identifier";
import type { Pos, Position } from "../../util/location";
import { isNewLine, lineBreakG } from "../../util/whitespace";
...
export default (superClass: Class<Parser>): Class<Parser> =>
  class extends superClass {
    ...
    constructor(options: ?Options, input: string) {
      let jsxTopLevel = options && options.jsxTopLevel;
      if (jsxTopLevel) {
        options.initialContextType = tc.j_expr;
      }
      super(options, input);
      this.jsxTopLevel = jsxTopLevel;
    }
      ...
      for (;;) {
        ...
        switch (ch) {
          case charCodes.lessThan:
            if (this.input.substr(this.state.pos, 4) === '<!--') {
              out += this.input.slice(chunkStart, this.state.pos);
              this.jsxSkipBlockComment();
    ...
    jsxSkipBlockComment(): void {
      const startLoc = this.state.curPosition();
      const start = this.state.pos;
      const end = this.input.indexOf('-->', (this.state.pos += 4));
      if (end === -1) this.raise(this.state.pos - 4, "Unterminated comment");
    ...
    jsxReadWord(): void {
      ...
      } while (isIdentifierChar(ch) || ch === charCodes.dash || isSpecialChar(ch));
    ...
    readToken(code: number): void {
      ...
      if (context === tc.j_oTag || context === tc.j_cTag) {
        if (isIdentifierStart(code) || isSpecialChar(code)) {
